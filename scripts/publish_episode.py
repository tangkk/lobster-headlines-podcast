...
def ensure_namespaces():
    ET.register_namespace('itunes','http://www.itunes.com/dtds/podcast-1.0.dtd'); ET.register_namespace('atom','http://www.w3.org/2005/Atom'); ET.register_namespace('content','http://purl.org/rss/1.0/modules/content/')
...
def add_episode(feed,enclosure_url,size,slug,title,description,duration):
    ...
    item=ET.Element('item'); ET.SubElement(item,'title').text=title; ET.SubElement(item,'description').text=description; ET.SubElement(item,'pubDate').text=format_datetime(dt.datetime.now(dt.timezone.utc)); ET.SubElement(item,'guid',{'isPermaLink':'false'}).text=slug; ET.SubElement(item,'enclosure',{'url':enclosure_url,'length':str(size),'type':'audio/mpeg'}); ET.SubElement(item,'{http://www.itunes.com/dtds/podcast-1.0.dtd}duration').text=duration; ET.SubElement(item,'{http://www.itunes.com/dtds/podcast-1.0.dtd}explicit').text='false'
...
def main():
    ...
    missing=[v for v in ['R2_ACCESS_KEY_ID','R2_SECRET_ACCESS_KEY','R2_ENDPOINT','R2_BUCKET','R2_PUBLIC_URL'] if not os.environ.get(v)]
    ...
    endpoint=os.environ['R2_ENDPOINT'].strip('"'); bucket=os.environ['R2_BUCKET']; public=os.environ['R2_PUBLIC_URL'].rstrip('/'); key=f"{a.prefix.rstrip('/')}/{a.slug}{os.path.splitext(a.audio)[1].lower() or '.mp3'}"; c=r2_client(endpoint); ensure_key_absent(c,bucket,key); size=os.path.getsize(a.audio); c.upload_file(a.audio,bucket,key,ExtraArgs={'ContentType':'audio/mpeg'}); add_episode(a.feed,f'{public}/{key}',size,a.slug,a.title,a.description,a.duration); print(f'uploaded {key} ({size} bytes); feed updated')

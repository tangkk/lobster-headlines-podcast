...
def verify_audio(url,expected_bytes,expected_duration):
    with tempfile.TemporaryDirectory() as d:
        p=os.path.join(d,'episode.mp3'); download(url+('&' if '?' in url else '?')+f'verify={int(time.time())}',p)
        if os.path.getsize(p)!=expected_bytes: raise RuntimeError('public MP3 size mismatch')
        s=round(float(subprocess.check_output(['ffprobe','-v','error','-show_entries','format=duration','-of','default=nw=1:nk=1',p],text=True).strip())); dur=f'{s//3600:02d}:{(s%3600)//60:02d}:{s%60:02d}'
        if dur!=expected_duration: raise RuntimeError(f'public MP3 duration mismatch: expected {expected_duration}, got {dur}')
def verify_feed(repo,slug,url,expected_bytes,expected_duration):
    ...
    for n in range(30):
        try:
            ...
            if e is None or e.attrib.get('url')!=url or int(e.attrib.get('length','-1'))!=expected_bytes: raise RuntimeError('live RSS enclosure mismatch')
            if i.findtext(f'{{{ITUNES}}}duration')!=expected_duration: raise RuntimeError('live RSS duration mismatch')
            return
